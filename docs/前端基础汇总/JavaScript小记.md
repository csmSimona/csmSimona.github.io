...
### :star:3、JS中数据类型的判断（ typeof，instanceof，constructor，Object.prototype.toString.call() ）
...
### :star:1、String对象中slice()、substring()、substr()的用法与区别
...
### :star:1、Array对象中slice() 、splice（）的区别
...
#### :star:2、JSONP：通过动态创建script，再请求一个带参网址实现跨域通信。
...
### :star:3、`setTimeout`、`requestAnimationFrame` 和 `requestIdleCallback` 有什么区别
